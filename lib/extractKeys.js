    ...
    objectKeys.forEach(async (key) => {
      if (Array.isArray(element[key])) {
        ...
        if (!alreadyCreatedOBjects[path + '.' + key]) {
          const roleType = getRoleType(element[key], write);
          await adapter
...
function getRoleType(element, write = false) {
  const ret = { role: 'state', type : typeof element};

  if (isDate(element)) {
    ret.role = 'date';
    if (ret.type !== 'number') ret.type = 'string';
    return ret;
  }
  if (isNumber(element) || ret.type === 'number') {
    if (write) ret.role = 'level';
    else ret.role = 'value';
    ret.type = 'number';
    return ret;
  }
  ...
  if (ret.type === 'boolean') {
    if (write) ret.role = 'switch';
    else ret.role =  'indicator';
    return ret;
  }
