 ...
 *
 * Ersetzt den alten NissanConnect-Login. Die eigentliche Kamereon-Vehicle-API
 * (car_adapter_base_url etc.) bleibt unverändert - hier geht es NUR um den
 * Login/Token-Teil.
 *
 * Benötigte Dependencies (vermutlich schon vorhanden):
 *   npm i axios tough-cookie http-cookie-agent
 *
 * WICHTIG: Die konkreten Werte für `settings` (client_id, scope, auth_base_url,
 * redirect_uri, auth_locale, auth_brand, auth_client, auth_platform,
 * kamereon_scope, user_base_url) stehen NICHT in kamereon.py, sondern in
 * kamereon_const.py (SETTINGS_MAP). Die müsst ihr noch abgleichen/ergänzen -
 * schick mir den Inhalt von kamereon_const.py, dann passe ich das an.
 */
